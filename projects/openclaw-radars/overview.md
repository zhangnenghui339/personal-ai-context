...
四个 subreddit 拆为独立任务避免超时拖垮整批。**2026-08-27：subreddit 切到 `SaaS / automation / AI_Agents / marketing`（去掉新闻/发烧友噪声大的 OpenAI/LocalLLaMA）；触发短语加 `automate / workflow for / stack for / recommendations for / do you use`；抓取预算 300s；SERP 核验上限 Top 20；cron 更名 `Band4Vacuum-*`（ID 不变）。**
